<template>
  <view class="page">
    <!-- 空状态 -->
    <view class="empty-state" v-if="dislikes.length === 0">
      <text class="empty-icon">🚫</text>
      <text class="empty-text">暂无避雷</text>
    ...
    <view class="product-grid" v-else>
      ...
      >
        ...
        <view class="delete-btn" @click.stop="removeDislike(item)">
          <text>删除</text>
        </view>
      </view>
    </view>
...
async function removeDislike(item) {
  try {
    const res = await post(`/api/products/${item.productId || item.id}/dislike`)
    if (res.code === 200) {
      dislikes.value = dislikes.value.filter(d => (d.productId || d.id) !== (item.productId || item.id))
      uni.showToast({ title: '已取消避雷', icon: 'success' })
    }
  } catch (e) {
